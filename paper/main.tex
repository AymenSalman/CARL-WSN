...
sub-100~ms delivery. Evaluated in MATLAB against seven baselines over 640
runs on a common lossy-channel model, CARL-WSN leads classical and clustering
baselines in pre-failure lifetime, comparable to the strongest energy-oriented
baselines; attains the lowest emergency latency among reactive and
learning-based baselines (24.2~ms, 25.6\% below AODV); and incurs overhead far
below flooding- and table-based protocols (0.53 vs.\ 14.82 for AODV), at a
modest delivery cost (90.2\% PDR). These results show that learning which
paradigm to use, not merely optimizing within one, is an effective operating
point for heterogeneous, latency-critical IoT-WSN deployments.
\end{abstract}
...
rather than from protocol-specific idealisations. Under this unified methodol-
ogy, CARL-WSN leads the classical and clustering baselines in network
lifetime (highest mean first-node-death round), performing comparably to
the strongest energy-oriented baselines, and attains the lowest emergency-traffic
latency among the reactive and learning-based protocols, while incurring rout-
ing overhead far below the flooding- and table-based protocols. These
gains are obtained by trading a modest, quantified reduction in packet
delivery, which we characterise explicitly.
\end{itemize}
...
\eIf{$U = A$}{
  ...
  \tcp{Step 3: Bounded critical-node deferral}
...
EH-Routing, MSLBA, and RLCR falls within seed-to-seed variation, indicating
statistically comparable performance rather than a distinguishable difference;
on lifetime, CARL-WSN is therefore best described as leading the classical and
clustering baselines while remaining statistically comparable to the
energy-oriented baselines (EH-Routing, MSLBA).
...
rapidly once cluster heads begin to fail. The HND gap over both
clustering protocols is statistically significant, as is the FND gap over FQ-UCR;
the FND difference from RLCR, however, falls within seed-to-seed variation,
indicating statistically comparable performance on this metric. On Class~A latency, CARL-WSN (24.2\,ms) is significantly lower than both RLCR
...
The results support a focused
set of claims. CARL-WSN leads the classical and clustering baselines in
pre-failure lifetime (highest mean FND, second-highest mean HND), with
results statistically comparable to the strongest energy-oriented baselines;
achieves the lowest emergency latency among the
reactive and learning-based protocols (24.2 ms, 25.6% below AODV); and
incurs routing overhead far below the flooding- and table-based protocols
(0.53 versus 2.31–14.82). These gains are obtained at a
...
\hline
Network lifetime (FND) & Highest mean (418); significantly above classical baselines \& FQ-UCR; comparable to EH-Routing, MSLBA, RLCR \\
Network lifetime (HND) & Second-highest mean (2198); significantly above classical baselines, RLCR, FQ-UCR \\
...
(380) and below RLCR (390) --- so that CARL-WSN's numerical edge over the lifetime-oriented baselines is
produced by the deferral policy that the urgency signal makes possible, not
by paradigm switching.

By weighting relay selection toward residual energy as well as progress,
and by bounding deferral at near-depleted nodes, CARL-WSN raises FND
to 418 — the highest mean of all eight protocols, numerically ahead of the
energy-harvesting (EH-Routing, 380) and clustering (RLCR, 390) baselines, and
significantly above the classical flat protocols by 28\%. On half-node-death,
CARL-WSN reaches HND = 2198, second only to MSLBA (2340) and significantly
above the classical protocols (1877) and both clustering protocols (1305 and
1172). The combination of energy-aware relaying and bounded deferral thus
delays both the onset and the progression of network failure, giving a
flat-routing protocol a lifetime profile statistically comparable to, and on
FND numerically exceeding, the dedicated lifetime-oriented baselines — while
retaining the low control overhead characteristic of flat routing relative to
