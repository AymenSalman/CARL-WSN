...
assumptions. Under mixed traffic, CARL-WSN extends network lifetime
beyond all baselines (first node death at round~418, the highest of
all protocols; half-node death at round~2198, second only to MSLBA),
...
packet, versus 14.8 for AODV and 2.31 for DSDV). These gains are
...
five points of the baselines, with energy balance and throughput
statistically comparable across all protocols. The learned Q-table
...
of one packet per node per round dominates its control budget.
...
failure. One-way ANOVA confirms the FND and HND differences are
statistically significant across protocols ($p < 0.001$).
...
\hline
\textbf{CARL-WSN} & \textbf{418} & 2198          & 90.2\% & 24.2          & 0.024 & 0.198          & 0.53          & 37.8 \\
AODV              & 327          & 1877          & 94.9\% & 32.5          & 0.024 & 0.196          & 14.82         & 36.5 \\
DSDV              & 327          & 1877          & 94.9\% & 24.4          & 0.024 & 0.196          & 2.31          & 36.5 \\
ZRP               & 327          & 1877          & 94.9\% & 28.2          & 0.024 & 0.196          & 5.91          & 36.5 \\
EH-Routing        & 380          & 2126          & 96.0\% & 24.4          & 0.024 & 0.203          & 1.00          & 38.0 \\
MSLBA             & 362          & \textbf{2340} & \textbf{96.9\%} & \textbf{21.1} & 0.024 & 0.193 & 1.00 & \textbf{48.5} \\
RLCR              & 390          & 1305          & 94.1\% & 34.0          & 0.024 & 0.200          & \textbf{0.04}$^\ddagger$ & 23.1 \\
FQ-UCR            & 123          & 1172          & 91.5\% & 35.2          & 0.024 & 0.198          & \textbf{0.04}$^\ddagger$ & 19.3 \\
\hline
...
energy expenditure (Fig.~\ref{fig:gini}), is likewise comparable
across protocols: values range narrowly from 0.193 (MSLBA) to 0.203
(EH-Routing), with CARL-WSN at 0.198. A one-way ANOVA does not find
these differences statistically significant ($p = 0.071$). We
therefore do not claim a balance advantage for CARL-WSN; its
energy-aware relay selection maintains balance comparable to the
baselines without concentrating consumption at particular nodes,
and in particular does not exhibit the cluster-head concentration
that the clustering protocols must manage through rotation.

\begin{figure}[!t]
\centering
\includegraphics[width=\textwidth]{fig4_energy_gini}
\caption{Gini coefficient of per-node energy consumption (lower is
more balanced). Differences across protocols are small and not
statistically significant ($p = 0.071$); CARL-WSN (0.198) is
comparable to all baselines.}
\label{fig:gini}
\end{figure}
...
(AODV/DSDV/ZRP at 36.5) and EH-Routing (38.0). MSLBA attains the
...
lowest throughput (RLCR 23.1, FQ-UCR 19.3); because throughput is
...
on-demand discovery; and 0.53 for the mixed workload, between the two
extremes in proportion to its 20/50/30 class mix. The baselines, by
...
On network lifetime, CARL-WSN leads: its FND (418) exceeds RLCR's
(390) and greatly exceeds FQ-UCR's (123), and its HND (2198) is far
above both clustering protocols (RLCR 1305, FQ-UCR 1172), which
degrade rapidly once cluster heads begin to fail. On Class~A
latency, CARL-WSN (24.2\,ms) is significantly lower than both RLCR
...
points below the baselines), while energy consumption, energy
balance, and throughput remain statistically comparable to the
baseline group.
...
Class~A latency        & 25.5\% below AODV; among the lowest overall \\
Routing overhead       & Far below AODV/ZRP/DSDV; above clustering \\
Packet delivery        & 90.2\% ($\approx$5 pts below baselines) \\
Energy / balance / throughput & Statistically comparable to baselines \\
\hline
\end{tabular}
\end{table}
...
The Q-table converges within 1150 rounds, requires only 216~bytes of
