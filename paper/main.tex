...
packet, versus 14.82 for AODV and 2.31 for DSDV). The lifetime gain stems from energy-aware relay selection
and bounded critical-node deferral, which the per-packet urgency signal
enables, while the learned paradigm switching contributes the
context-adaptive overhead and the emergency-latency protection. These gains are
...
\hline
$w_e/w_p$ (0.4/0.6)          & 0.6 & 0.0 & 4.7 & 1.0 & 0.4 \\
...
sensitivity to the channel sharpness ($\sim$15\% FND swing) and none to the
...
Throughput & Comparable to classical and EH-Routing; above clustering; below MSLBA \\
\hline
\end{tabular}
\end{table}
...
progress alone without regard to residual energy. It is therefore the
relay-selection and deferral mechanisms, rather than the choice of
routing paradigm itself, that account for CARL-WSN's lifetime
advantage; this is confirmed directly by Table~\ref{tab:defer_sweep},
where disabling deferral lowers FND to 381 --- on par with EH-Routing
(380) and below RLCR (390) --- so that the lead over the
lifetime-oriented baselines is produced by the deferral policy that
the urgency signal makes possible, not by paradigm switching.
...
energy-oriented baselines EH-Routing and MSLBA. This lifetime advantage
is produced by the energy-aware relay selection and bounded deferral
that the per-packet urgency signal enables, rather than by paradigm
switching itself: as Table~\ref{tab:defer_sweep} shows, disabling
deferral returns FND to the level of the energy-oriented baselines.
